// Author: LE TRUNG HIEU (A0161308M)
...
class Alice { // Alice is a TCP  client
    ...
    public Alice(String ipStr, String portStr) {
        ...
        try {
        	Socket clientSocket = new Socket(ipStr, Integer.parseInt(portStr));
    ...
    public void sendSessionKey() {
    	try {
        	SealedObject encryptedSessionKeyObj = (this.crypto).getSessionKey();
        	this.toBob.writeObject(encryptedSessionKeyObj);
        } catch (IOException e) {
            System.out.println("Error sending key to Bob");
            System.exit(1);
        }
    }
    ...
    public void receiveMessages() {
        ...
        try {
            writer = new PrintWriter(MESSAGE_FILE, "UTF-8");
	    	while(true) {
	    		System.out.println("Received");
	    		SealedObject encryptedMsgObj = (SealedObject)this.fromBob.readObject();
            	String encryptedMsg = this.crypto.decryptMessage(encryptedMsgObj);
